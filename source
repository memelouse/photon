...
Script.Functions.GetClosestBodyPart = function(Char)
    ...
    for _, v in pairs(Parts) do
        if Photon.Silent.UseWhitelistedParts and table.find(Photon.Silent.WhitelistedPart, v.Name) == nil then continue end
...
Script.Functions.GetClosestPointOnPart = function(Part)
    ...
    return Part.CFrame * Vector3.new(
        math.clamp(Transform.X, - Part.Size.X % (Photon.Silent.ClosestPointScale / 2), Part.Size.X % (Photon.Silent.ClosestPointScale / 2)), 
        math.clamp(Transform.Y, - Part.Size.Y % (Photon.Silent.ClosestPointScale / 2), Part.Size.Y % (Photon.Silent.ClosestPointScale / 2)), 
        math.clamp(Transform.Z, - Part.Size.Z % (Photon.Silent.ClosestPointScale / 2), Part.Size.Z % (Photon.Silent.ClosestPointScale / 2))
    )
end

-- // Mod Detection Function
Script.Functions.CheckIfMod = function(Plr)
    if Photon.ModDetection.Enabled then
        if (game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Creator.CreatorType == "Group" and true or false) == true then
            ...
            if Plr:IsInGroup(GroupId) and Plr:GetRankInGroup(GroupId) > Photon.ModDetection.Rank then
                if Photon.ModDetection.Method == ("Kick") then 
                    task.wait(Photon.ModDetection.Delay)
                    Client:Kick("Detected Moderator / Admin: " .. tostring(Plr))
                elseif Photon.ModDetection.Method == ("Notification") then
...
Script.Functions.GetClosestPlayer = function(Mode)
    local Target, Closest, HitChance = nil, math.huge, Script.Functions.CalculateChance(Photon.Silent.HitChance)
    ...
    if Mode then
        for _, v in pairs(Players:GetPlayers()) do
            if v.Character and v ~= Client and v.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if Photon.UniversalCheck.WallCheck and not Script.Functions.WallCheck(v.Character.HumanoidRootPart.Position, v.Character) then continue end
                local Distance = Script.Functions.GetMagnitudeFromMouse(v.Character.HumanoidRootPart)
                if (Photon.Silent.ForceLock and Script.Drawing.SilentCircle.Radius + (Distance * 0.3) < Distance) then continue end
        ...
        if Photon.Silent.ForceLock_AimAssistTarget and Script.Functions.Alive(AimTarget) then
            Target = AimTarget
        elseif Photon.Silent.ForceLock_AimAssistTarget == false then
            Target = ForceLock
        end
    end
...
Script.Functions.SilentCheck = function(Plr)
    if Photon.UniversalCheck.KoCheck and Plr.Character:FindFirstChild("BodyEffects") then
        local KoCheck = false
    ...
    if Photon.UniversalCheck.FriendCheck and table.find(Script.Friends, Plr) then
        return false  
    end
    if Photon.UniversalCheck.TargetDeathCheck and Plr.Character:FindFirstChild("Humanoid") and Plr.Character.Humanoid.health < 4 then
        return false 
    end
    if Photon.UniversalCheck.ForceFieldCheck and Plr.Character:FindFirstChildOfClass("ForceField") then
        return false
    end
    if Photon.UniversalCheck.InVisibleCheck and Plr.Character:FindFirstChild("Head") and Plr.Character.Head.Transparency > 0.5 then
        return false  
    end
    if Photon.UniversalCheck.CrewCheck and Script.Functions.FindCrew(Plr) and Plr.DataFolder.Information:FindFirstChild("Crew").Value ~= Client.DataFolder.Information:FindFirstChild("Crew").Value then 
        return false  
    end
    if Photon.UniversalCheck.TeamCheck and Client.Team ~= nil and Plr.Team ~= nil and Plr.Team == Client.Team then 
        return false 
    end
    return true
end
...
Script.Functions.GetClosestPartMethod = function(Plr)
    ...
    if Photon.Silent.LegitMode then
        if Photon.Silent.ClosestPart == false and Plr.Character:FindFirstChild(Photon.Silent.Part) then
            return Plr.Character[Photon.Silent.Part]
        elseif AimTarget and Photon.AimAssist.ClosestPart and Plr.Character:FindFirstChild(Photon.AimAssist.Part) then
            return Plr.Character[Photon.AimAssist.Part]
        end
        return nil
    end
    ...
    if Script.Functions.Alive(Plr) then
        if SilentTarget.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
            local AirHitChance = Script.Functions.CalculateChance(Photon.Silent.AirHitChance)
            if not AirHitChance then
                return nil
            end
            if Photon.Silent.UseAirPart and SilentTarget.Character:FindFirstChild(Photon.Silent.AirPart) then
                return SilentTarget.Character[Photon.Silent.AirPart]
            end
        else
            if Photon.Silent.UseAirPart and SilentTarget.Character:FindFirstChild(Script.SavedValue.SilentPart) then
                return SilentTarget.Character[Script.SavedValue.SilentPart]
            end
        end
        
        if Photon.Silent.ClosestPart == false and Plr.Character:FindFirstChild(Photon.Silent.Part) then
            local PartDistance = Script.Functions.GetMagnitudeFromMouse(Plr.Character[Photon.Silent.Part])

            if (Photon.Silent.ForceLock == false and Script.Drawing.SilentCircle.Radius + 4 > PartDistance) or Photon.Silent.ForceLock == true then
                return Plr.Character[Photon.Silent.Part]
            end
            return nil
        end
        ...
        for _, v in pairs(Parts) do
            ...
            if v:IsA("MeshPart") and v:IsA("BasePart") and Script.Functions.OnScreen(v) then
                ...
                for _, v in pairs(Filterd) do
                    ...
                    if (Photon.Silent.ForceLock == false and Photon.Silent.LegitMode == false and Script.Drawing.SilentCircle.Radius + 4 < Mag) then continue end
...
Script.Functions.GetClosestPlayer2 = function()
    ...
    for _, v in pairs(Players:GetPlayers()) do
        if v.Character and v ~= Client and v.Character:FindFirstChild("HumanoidRootPart") then
            if not Script.Functions.OnScreen(v.Character.HumanoidRootPart) then continue end
            if Photon.UniversalCheck.WallCheck and not Script.Functions.WallCheck(v.Character.HumanoidRootPart.Position, v.Character) then continue end
        	if Photon.UniversalCheck.InVisibleCheck and v.Character:FindFirstChild("Head") then
        		if v.Character.Head.Transparency > 0.5 then
        			continue
        		end
        	end
            if Photon.UniversalCheck.ForceField and not v.Character:FindFirstChildOfClass("ForceField") then
                continue
            end
        	if Photon.UniversalCheck.CrewCheck and Script.Functions.FindCrew(v) and v.DataFolder.Information:FindFirstChild("Crew").Value ~= Client.DataFolder.Information:FindFirstChild("Crew").Value then
        		continue
        	end
            if Photon.UniversalCheck.TeamCheck and Client.Team ~= nil and v.Team ~= nil and v.Team ~= Client.Team then
                continue
            end
            if Photon.UniversalCheck.FriendCheck and table.find(Script.Friends, v) ~= nil then
                continue
            end
            local Distance = Script.Functions.GetMagnitudeFromMouse(v.Character.HumanoidRootPart)
            if Distance < Closest then
                if (Photon.AimAssist.UseCircleRadius and Script.Drawing.AimAssistCircle.Radius + (Distance * 0.3) < Distance) then continue end
                Closest = Distance
                Target = v
            end
        end
...
Script.Functions.GetPartPosition = function(Plr)
    ...
    if Photon.Silent.ClosestPoint then
        TargetCF = ClosestPointCF
    else
        if Plr.Character:FindFirstChild(Photon.Silent.Part) then
            TargetCF = Plr.Character[Photon.Silent.Part].Position
        end
    end

    if Photon.Silent.AntiGroundShots and CurrentVelocity.Y < Photon.Silent.AntiGroundActivation then
        TargetFalling = true
    end
    if TargetCF and CurrentVelocity then
        SilentPos = TargetCF
        if Photon.Silent.PredictMovement then 
            if Photon.Silent.BlatantMode then
                local Enabled = true
                ...
                if Enabled and Photon.UniversalCheck.WallCheck_V2 and not Script.Functions.WallCheck(Plr.Character.HumanoidRootPart.Position + (SilentTarget.Character.HumanoidRootPart.Velocity * Photon.Silent.Prediction), Plr.Character) then
                    return
                end
                if Enabled then
                    if TargetFalling then
                        SilentPos = SilentPos + (Vector3.new(SilentVel.X, (SilentVel.Y * Photon.Silent.AntiGroundValue), SilentVel.Z) * Photon.Silent.Prediction)
                    else
                        SilentPos = SilentPos + (SilentVel * Photon.Silent.Prediction)
                    end
                else
                    if TargetFalling then
                        SilentPos = SilentPos + (Vector3.new(CurrentVelocity.X, (CurrentVelocity.Y * Photon.Silent.AntiGroundValue), CurrentVelocity.Z) * Photon.Silent.Prediction)
                    else
                        SilentPos = SilentPos + (CurrentVelocity * Photon.Silent.Prediction)
                    end
                end
            else
                if TargetFalling then
        ...
        if Photon.Silent.Humanize then
            local HumanizeValue = Photon.Silent.HumanizeValue 
    ...
    if SilentPos then
        if Photon.Silent.LegitMode then
            ...
            if (Photon.Silent.ForceLock == false and Script.Drawing.SilentCircle.Radius + 4 > Magnitude) or Photon.Silent.ForceLock == true then
                return SilentPos
            end
        else
            return SilentPos
...
MousePosChanger2 = hookmetamethod(game, "__index", function(self, Index)
    if not checkcaller() and Photon.Silent.Enabled and Photon.Silent.AntiAimViewer == false and self == Mouse and Script.Functions.Alive(SilentTarget) then
        if Index == ("Hit") then
            local EndPoint = Script.Functions.GetPartPosition(SilentTarget)
            if EndPoint then
                return EndPoint
            end
        elseif Index == ("Target") and SilentTarget.Character:FindFirstChild(Photon.Silent.Part) then 
            return SilentTarget.Character[Photon.Silent.Part]
        end
    end
...
Script.Functions.MouseChanger = function()
    if Photon.AimAssist.Enabled and not Uis:GetFocusedTextBox() and Script.Functions.Alive(AimTarget) and Script.Functions.Alive(Client) and AimTarget.Character:FindFirstChild(Photon.AimAssist.Part) then
        local EndPosition = nil
        local TargetPos = AimTarget.Character[Photon.AimAssist.Part].Position

        if Photon.AimAssist.ThirdPerson and Photon.AimAssist.FirstPerson == false then
            if (Camera.Focus.p - Camera.CoordinateFrame.p).Magnitude < 1 then
                return
            end
        elseif Photon.AimAssist.ThirdPerson == false and Photon.AimAssist.FirstPerson then
            if (Camera.Focus.p - Camera.CoordinateFrame.p).Magnitude > 1 then
                return
            end
        end
        ...
        if Photon.AimAssist.EnableChance then
            local Chance = Script.Functions.CalculateChance(Photon.AimAssist.Chance)
            if not Chance then
                return
            end
        end
        if Photon.UniversalCheck.ToolOut and not Client.Character:FindFirstChildWhichIsA("Tool") then
            return
        end
        if Photon.UniversalCheck.ForceFieldCheck and AimTarget.Character:FindFirstChildOfClass("ForceField") then
            AimTarget = nil
            CurrentVelocity2 = Vector3.zero
            return
        end
        if Photon.AimAssist.RandomPart or Photon.AimAssist.ClosestPart then
            if Script.Functions.OnScreen(AimTarget.Character.HumanoidRootPart) then
                if Photon.AimAssist.RandomPart then
                    local RandomizedPart = Script.Functions.GetRandomBodyPart(AimTarget.Character)
                    if RandomizedPart ~= nil then
                        Photon.AimAssist.Part = tostring(RandomizedPart)
                    end
                elseif Photon.AimAssist.ClosestPart then
                    local ClosestPart = Script.Functions.GetClosestBodyPart(AimTarget.Character)
                    Photon.AimAssist.Part = tostring(ClosestPart)
                end
            else
                if Photon.AimAssist.Method == ("Mouse") then
                    return
                end
            end
        end
        if Photon.UniversalCheck.ReloadCheck then
        ...
        if Photon.UniversalCheck.NoAmmoCheck and Client.Character:FindFirstChildWhichIsA("Tool") and Client.Character:FindFirstChildWhichIsA("Tool"):FindFirstChild("Ammo") then
            if Client.Character:FindFirstChildWhichIsA("Tool"):FindFirstChild("Ammo").Value <= 0 then
                return
            end
        end
        if Photon.AimAssist.Advanced.WallCheck_V2 and not Script.Functions.WallCheck(AimTarget.Character.HumanoidRootPart.Position, AimTarget.Character) then 
            return
        end
        if Photon.AimAssist.DisableOutSideCircle then
        ...
        if Photon.UniversalCheck.PlayerDeathCheck then
        ...
        if Photon.UniversalCheck.KoCheck and AimTarget.Character:FindFirstChild("BodyEffects") then 
        ...
        if Photon.UniversalCheck.TargetDeathCheck then
        ...
        if Photon.AimAssist.Shake.AirShake or Photon.AimAssist.AirSmoothness then
            if AimTarget.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
                if Photon.AimAssist.AirSmoothness then
                    Photon.AimAssist.Smoothness_X = Photon.AimAssist.AirSmoothness_X
                    Photon.AimAssist.Smoothness_X = Photon.AimAssist.AirSmoothness_Y
                end
                if Photon.AimAssist.Shake.AirShake then
                    if Photon.AimAssist.Shake.Shake_X == Script.SavedValue.ShakeX then
                        Photon.AimAssist.Shake.Shake_X = Photon.AimAssist.Shake.Shake_X * (Photon.AimAssist.Shake.AirPercentage / 100)
                    end
                    if Photon.AimAssist.Shake.Shake_Y == Script.SavedValue.ShakeY then
                        Photon.AimAssist.Shake.Shake_Y = Photon.AimAssist.Shake.Shake_Y * (Photon.AimAssist.Shake.AirPercentage / 100)
                    end
                    if Photon.AimAssist.Shake.Shake_Z == Script.SavedValue.ShakeZ then
                        Photon.AimAssist.Shake.Shake_Z = Photon.AimAssist.Shake.Shake_Z * (Photon.AimAssist.Shake.AirPercentage / 100)
                    end
                end
            else
                if Photon.AimAssist.AirSmoothness then
                    if Photon.AimAssist.Smoothness_X ~= Script.SavedValue.AimAssistSmoothX then
                        Photon.AimAssist.Smoothness_X = Script.SavedValue.AimAssistSmoothX
                    end
                    if Photon.AimAssist.Smoothness_Y ~= Script.SavedValue.AimAssistSmoothY then
                        Photon.AimAssist.Smoothness_Y = Script.SavedValue.AimAssistSmoothY
                    end
                end
                if Photon.AimAssist.Shake.AirShake then
                    if Photon.AimAssist.Shake.Shake_X ~= Script.SavedValue.ShakeX then
                        Photon.AimAssist.Shake.Shake_X = Script.SavedValue.ShakeX
                    end
                    if Photon.AimAssist.Shake.Shake_Y ~= Script.SavedValue.ShakeY then
                        Photon.AimAssist.Shake.Shake_Y = Script.SavedValue.ShakeY
                    end
                    if Photon.AimAssist.Shake.Shake_Z ~= Script.SavedValue.ShakeZ then
                        Photon.AimAssist.Shake.Shake_Z = Script.SavedValue.ShakeZ
                    end
                end
            end
        end
        
        if FrameSkip and Photon.AimAssist.FrameSkip.TargetPart.Enabled and AimTarget.Character:FindFirstChild(Photon.AimAssist.FrameSkip.TargetPart.Part) then
            if Photon.AimAssist.FrameSkip.UsePrediction then
                EndPosition = AimTarget.Character[Photon.AimAssist.FrameSkip.TargetPart.Part].Position + (CurrentVelocity2 * Photon.AimAssist.Prediction)
            else
                EndPosition = AimTarget.Character[Photon.AimAssist.FrameSkip.TargetPart.Part].Position
            end
        elseif FrameSkip then
            if Photon.AimAssist.FrameSkip.UsePrediction then
                EndPosition = TargetPos + (CurrentVelocity2 * Photon.AimAssist.Prediction)
            else
                EndPosition = TargetPos
            end
        elseif Photon.AimAssist.PredictMovement then
            EndPosition = TargetPos + (CurrentVelocity2 * Photon.AimAssist.Prediction)
        else
            EndPosition = TargetPos
        end

        if (EndPosition and (tick() - LastStutter) >= (Photon.AimAssist.Advanced.Stutter / 1000)) then
            LastStutter = tick()
            if Photon.AimAssist.Shake.Enabled and FrameSkip == false then
                local Mag = math.ceil((EndPosition - Client.Character.HumanoidRootPart.Position).Magnitude)
                EndPosition = EndPosition + (Vector3.new(
                    math.random(-Mag * Photon.AimAssist.Shake.Shake_X, Mag * Photon.AimAssist.Shake.Shake_X), 
                    math.random(-Mag * Photon.AimAssist.Shake.Shake_Y, Mag * Photon.AimAssist.Shake.Shake_Y), 
                    math.random(-Mag * Photon.AimAssist.Shake.Shake_Z, Mag * Photon.AimAssist.Shake.Shake_Z)) / 1000
                )
            end
            if Photon.AimAssist.Method == ("Mouse") then
                local Vec2Pos = Camera:WorldToScreenPoint(EndPosition)
                local InCrementX = (Vec2Pos.X - Photon.Silent.Fov.Offset.X) * (Photon.AimAssist.UseSmoothness and Photon.AimAssist.Smoothness_X or 1)
                local InCrementY = (Vec2Pos.Y - Photon.Silent.Fov.Offset.Y) * (Photon.AimAssist.UseSmoothness and Photon.AimAssist.Smoothness_Y or 1)
                if FrameSkip then
                    FrameSkip = false
                    InCrementX = ((Vec2Pos.X - Photon.Silent.Fov.Offset.X) - Mouse.X) * Photon.AimAssist.FrameSkip.Power
                    InCrementY = ((Vec2Pos.Y - Photon.Silent.Fov.Offset.Y) - Mouse.Y) * Photon.AimAssist.FrameSkip.Power
                end
                mousemoverel(InCrementX, InCrementY)
            elseif Photon.AimAssist.Method == ("Camera") then
                local Vec3Pos = CFrame.new(Camera.CFrame.p, EndPosition)
                if FrameSkip then
                    FrameSkip = false
                    Camera.CFrame = Camera.CFrame:Lerp(Vec3Pos, Photon.AimAssist.FrameSkip.Power, Photon.AimAssist.Advanced.EasingStyle, Photon.AimAssist.Advanced.EasingDirection)
                    return
                end
                Camera.CFrame = Camera.CFrame:Lerp(Vec3Pos, (Photon.AimAssist.UseSmoothness and Photon.AimAssist.Smoothness_X or 1), Photon.AimAssist.Advanced.EasingStyle, Photon.AimAssist.Advanced.EasingDirection)
            end
        end
    end
end
...
Script.Functions.SilentMisc = function()
    if Photon.Silent.PingPrediction.Enabled and Photon.Silent.PredictMovement and Photon.Silent.GunSettings.Methods.Prediction == false and game:GetService("Stats") and game:GetService("Stats"):FindFirstChild("Network") and game:GetService("Stats").Network:FindFirstChild("ServerStatsItem") and game:GetService("Stats").Network.ServerStatsItem:FindFirstChild("Data Ping") then
        local Ping = math.floor(game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue())
        if Photon.Silent.PingPrediction.AutoMatic == false then
            if Ping > 200 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P200_Inf
            elseif Ping > 190 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P190_200
            elseif Ping > 180 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P180_190
            elseif Ping > 170 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P170_180
            elseif Ping > 160 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P160_170
            elseif Ping > 150 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P150_160
            elseif Ping > 140 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P140_150
            elseif Ping > 130 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P130_140
            elseif Ping > 120 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P120_130
            elseif Ping > 110 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P110_120
            elseif Ping > 100 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P100_110
            elseif Ping > 90 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P90_100
            elseif Ping > 80 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P80_90
            elseif Ping > 70 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P70_80
            elseif Ping > 60 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P60_70
            elseif Ping > 50 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P50_60
            elseif Ping > 40 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P40_50
            elseif Ping > 30 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P30_40
            elseif Ping > 20 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P20_30
            elseif Ping > 10 then
                Photon.Silent.Prediction = Photon.Silent.PingPrediction.P10_20
            end
        end
    end
end
...
Script.Functions.GetSilentTarget = function()
    local Target = Script.Functions.GetClosestPlayer(not Photon.Silent.ForceLock)

    if Photon.Silent.AntiAimViewer == false and Script.Functions.Alive(Target) and Script.Functions.SilentCheck(Target) then 
        SilentTarget = Target
        local GetPart = Script.Functions.GetClosestPartMethod(Target)
        if GetPart ~= nil then
            Photon.Silent.Part = tostring(GetPart)
...
Script.Functions.Desync = function()
    if Desync and Script.Functions.Alive(Client) and Client.Character.Humanoid.health > Photon.Desync.HealthDeActivation then
        if Photon.Desync.Method == ("Freeze_Pos") and sethiddenproperty then
            FreezePos = not FreezePos
            sethiddenproperty(Client.Character.HumanoidRootPart, "NetworkIsSleeping", FreezePos)
        elseif Photon.Desync.Method == ("Slow_Data") and setfflag then
            ...
            if Photon.Desync.Method == ("Vel_StandBy") then
            ...
            elseif Photon.Desync.Method == ("Vel_Multi") then
                Client.Character.HumanoidRootPart.Velocity = Client.Character.HumanoidRootPart.Velocity * Photon.Desync.Power
            elseif Photon.Desync.Method == ("Custom_Vel") then
                Client.Character.HumanoidRootPart.Velocity = Vector3.new(Photon.Desync.Custom.Vel_X, Photon.Desync.Custom.Vel_Y, Photon.Desync.Custom.Vel_Z)
            elseif Photon.Desync.Method == ("Vel_Under") then
                Client.Character.HumanoidRootPart.Velocity = Vector3.new(0, -Photon.Desync.Power, 0)
            elseif Photon.Desync.Method == ("Vel_Over") then
                Client.Character.HumanoidRootPart.Velocity = Vector3.new(0, Photon.Desync.Power, 0)
            elseif Photon.Desync.Method == ("Vel_Zero") then
                Client.Character.HumanoidRootPart.Velocity = Vector3.zero
            end
            
            if Photon.Desync.Visualize.Enabled then
                ...
                if OnScreen then
                    ...
                    Script.Drawing.DesyncCircle.Color = Photon.Desync.Visualize.Color
                    Script.Drawing.DesyncCircle.Radius = (Photon.Desync.Visualize.Radius * 10) / GetMag
...
Script.Functions.UpdateFOV = function()
    local GetScreenPos = Script.Functions.GetFovPosition()
    Script.Drawing.AimAssistCircle.Visible = Photon.AimAssist.Fov.Visible
    Script.Drawing.AimAssistCircle.Filled = Photon.AimAssist.Fov.Filled
    Script.Drawing.AimAssistCircle.Color = Photon.AimAssist.Fov.Color
    Script.Drawing.AimAssistCircle.Transparency = Photon.AimAssist.Fov.Transparency
    if Photon.Silent.Fov.Method == ("Screen") then
        Script.Drawing.AimAssistCircle.Position = Vector2.new(GetScreenPos.X, GetScreenPos.Y) - Photon.Silent.Fov.Offset
    else
        Script.Drawing.AimAssistCircle.Position = Vector2.new(GetScreenPos.X, GetScreenPos.Y + GuiS:GetGuiInset().Y) - Photon.Silent.Fov.Offset
    end

	Script.Drawing.AimAssistCircle.Radius = Photon.AimAssist.Fov.Radius * 3
    
    Script.Drawing.SilentCircle.Visible = Photon.Silent.Fov.Visible
    Script.Drawing.SilentCircle.Color = Photon.Silent.Fov.Color
    Script.Drawing.SilentCircle.Filled = Photon.Silent.Fov.Filled
    Script.Drawing.SilentCircle.Transparency = Photon.Silent.Fov.Transparency
    if Photon.Silent.Fov.StickyFov and Script.Functions.Alive(SilentTarget) and SilentTarget.Character:FindFirstChild(Photon.Silent.Part) then
        local PartPos, OnScreen = Camera:WorldToViewportPoint(SilentTarget.Character[Photon.Silent.Part].Position)
        if OnScreen then
            local Magnitude = ((Vector2.new(PartPos.X, PartPos.Y) - Vector2.new(GetScreenPos.X, GetScreenPos.Y + GuiS:GetGuiInset().Y)) - Photon.Silent.Fov.Offset).Magnitude

            if (Photon.Silent.ForceLock == false and Script.Drawing.SilentCircle.Radius + 4 > Magnitude) or Photon.Silent.ForceLock == true then
                Script.Drawing.SilentCircle.Position = Vector2.new(PartPos.X, PartPos.Y) - Photon.Silent.Fov.Offset
            else
                if Photon.Silent.Fov.Method == ("Screen") then
                    Script.Drawing.SilentCircle.Position = Vector2.new(GetScreenPos.X, GetScreenPos.Y) - Photon.Silent.Fov.Offset
                else
                    Script.Drawing.SilentCircle.Position = Vector2.new(GetScreenPos.X, GetScreenPos.Y + GuiS:GetGuiInset().Y) - Photon.Silent.Fov.Offset
                end
            end
        else
            if Photon.Silent.Fov.Method == ("Screen") then
...
Script.Functions.UpdateEsp = function()
    for i, v in pairs(Script.EspPlayers) do
        if Photon.Esp.Enabled and i.Character and i.Character:FindFirstChild("Humanoid") and i.Character:FindFirstChild("HumanoidRootPart") and i.Character:FindFirstChild("Head") then
            ...
            if OnScreen then
                if Photon.Esp.Name.Enabled then
                    v.Name.Position = Vector2.new(BoxSize.X / 2 + BoxPos.X, BoxPos.Y - 16)
                    v.Name.Outline = Photon.Esp.Name.OutLine
                    v.Name.Text = Hum.DisplayName
                    v.Name.Color = Photon.Esp.Name.Color
                    v.Name.OutlineColor = Color3.fromRGB(0, 0, 0)
                    v.Name.Font = 0
                    v.Name.Size = Photon.Esp.TextSize
                ...
                if Photon.Esp.Distance.Enabled and Client.Character and Client.Character:FindFirstChild("HumanoidRootPart") then
                    v.Distance.Position = Vector2.new(BoxSize.X / 2 + BoxPos.X, BottomOffset)
                    v.Distance.Outline = Photon.Esp.Distance.OutLine
                    v.Distance.Text = "[" .. math.floor((Hrp.Position - Client.Character.HumanoidRootPart.Position).Magnitude) .. "m]"
                    v.Distance.Color = Photon.Esp.Distance.Color
                    v.Distance.OutlineColor = Color3.fromRGB(0, 0, 0)

                    v.Distance.Font = 0
                    v.Distance.Size = Photon.Esp.TextSize
                ...
                if Photon.Esp.Tool.Enabled then
                    if Photon.Esp.Distance.Enabled then
                    ...
                    v.Tool.Outline = Photon.Esp.Tool.OutLine
                    ...
                    v.Tool.Color = Photon.Esp.Tool.Color
                    v.Tool.OutlineColor = Color3.fromRGB(0, 0, 0)
                    
                    v.Tool.Font = 0
                    v.Tool.Size = Photon.Esp.TextSize
                ...
                if Photon.Esp.Box.Enabled then
                    v.BoxOutline.Size = BoxSize
                    v.BoxOutline.Position = BoxPos
                    v.BoxOutline.Visible = Photon.Esp.Box.OutLine
                    v.BoxOutline.Color = Color3.fromRGB(0, 0, 0)
                    
                    v.Box.Size = BoxSize
                    v.Box.Position = BoxPos
                    if Photon.Esp.TargetColor.Enabled and SilentTarget ~= nil and i == SilentTarget then
                        v.Box.Color = Photon.Esp.TargetColor.Color
                    elseif Photon.Esp.CrewColor.Enabled and Script.Functions.FindCrew(i) and i.DataFolder.Information:FindFirstChild("Crew").Value == Client.DataFolder.Information:FindFirstChild("Crew").Value then
                        v.Box.Color = Photon.Esp.CrewColor.Color
                    else
                        v.Box.Color = Photon.Esp.Box.Color
                    end
                    v.Box.Visible = true
                else
                ...
                if Photon.Esp.HealthBar.Enabled then
                    if Photon.Esp.HealthBar.HealthColor then
                        local Health = i.Character.Humanoid.Health / i.Character.Humanoid.MaxHealth
                        v.HealthBar.Color = Color3.fromHSV(Health * 0.3, 1, 1) 
                    else
                        v.HealthBar.Color = Photon.Esp.HealthBar.Color
                    end
                    ...
                    v.HealthBarOutline.Visible = Photon.Esp.HealthBar.OutLine
                else
                    v.HealthBarOutline.Visible = false
                    v.HealthBar.Visible = false
                end

                if Photon.Esp.HealthText.Enabled then
                    local Offset = 22
                    if Photon.Esp.ArmorBar.Enabled == false then
                        Offset = Offset - 7
                    end
                    if Photon.Esp.HealthBar.Enabled == false then
                        Offset = Offset - 7
                    end

                    if Photon.Esp.HealthText.HealthColor then
                        local Health = i.Character.Humanoid.Health / i.Character.Humanoid.MaxHealth
                        v.HealthText.Color = Color3.fromHSV(Health * 0.3, 1, 1) 
                    else
                        v.HealthText.Color = Photon.Esp.HealthText.Color
                    end
                    ...
                    v.HealthText.Outline = Photon.Esp.HealthText.OutLine

                    v.HealthText.Font = 0
                    v.HealthText.Size = Photon.Esp.TextSize
                ...
                if Photon.Esp.Flags.Enabled then
                    local Offset = 10
                    if Photon.Esp.ArmorBar.Enabled == false then
                        Offset = Offset - 7
                    end
                    ...
                    if i.Character.HumanoidRootPart.Velocity.Magnitude > 120 and Photon.Esp.Flags.DesyncState then
                        v.Flag.Text = "Desyncing"
                    elseif i.Character.HumanoidRootPart.Velocity.Y > 2 and Photon.Esp.Flags.WalkingState then
                        v.Flag.Text = "Jumping"
                    elseif i.Character.HumanoidRootPart.Velocity.Y < -2 and Photon.Esp.Flags.WalkingState then
                        v.Flag.Text = "Falling"
                    elseif i.Character.HumanoidRootPart.Velocity.Magnitude > 2 and Photon.Esp.Flags.WalkingState then
                        v.Flag.Text = "Walking"
                    elseif i.Character.HumanoidRootPart.Velocity.Magnitude < 1 and Photon.Esp.Flags.WalkingState then
                    ...
                    v.Flag.Color = Photon.Esp.Flags.Color
                    v.Flag.OutlineColor = Color3.fromRGB(0, 0, 0)
                    v.Flag.Outline = Photon.Esp.Flags.OutLine

                    v.Flag.Font = 0
                    v.Flag.Size = Photon.Esp.TextSize
                ...
                if Photon.Esp.Tracer.Enabled then
                    if Photon.Esp.Tracer.Method == ("Screen") then
                    ...
                    v.Tracer.Thickness = Photon.Esp.Tracer.Thickness
                    v.Tracer.Color = Photon.Esp.Tracer.Color
                ...
                if i.Character:FindFirstChild("BodyEffects") and i.Character:FindFirstChild("BodyEffects"):FindFirstChild("Armor") then
                    if Photon.Esp.ArmorBar.Enabled then
                        if Photon.Esp.HealthBar.Enabled then
                        ...
                        v.ArmorBar.Color = Photon.Esp.ArmorBar.Color
                        ...
                        v.ArmorBarOutline.Visible = Photon.Esp.ArmorBar.OutLine
                    else
                        v.ArmorBarOutline.Visible = false
                        v.ArmorBar.Visible = false
                    end
                    if Photon.Esp.ArmorText.Enabled then
                        local Offset = 22
                        if Photon.Esp.ArmorBar.Enabled == false then
                            Offset = Offset - 7
                        end
                        ...
                        v.ArmorText.Color = Photon.Esp.ArmorText.Color
                        v.ArmorText.OutlineColor = Color3.fromRGB(0, 0, 0)
                        v.ArmorText.Outline = Photon.Esp.ArmorText.OutLine

                        v.ArmorText.Font = 0
                        v.ArmorText.Size = Photon.Esp.TextSize
...
Client.Chatted:Connect(function(Msg)
    if Msg == Photon.ChatCommands.CrashMode then
        if Photon.ChatCommands.CrashMethod == ("Freeze") then
            while true do end
        elseif Photon.ChatCommands.CrashMethod == ("Shutdown") then
            game:Shutdown()
        end
    elseif Msg == Photon.ChatCommands.RejoinServer then
        game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, game.JobId, Client)
    elseif Msg == Photon.ChatCommands.RandomServer then
        game:GetService("TeleportService"):Teleport(game.PlaceId, Client) 
    end
    local Splitted = string.split(Msg, " ")
    if Splitted[1] and Splitted[2] and Photon.ChatCommands.Enabled then
        if Splitted[1] == Photon.ChatCommands.LoadConfig then
            if isfolder("Photon/" .. game.PlaceId) and isfolder("Photon/" .. game.PlaceId .. "/Configs") and isfile("Photon/" .. game.PlaceId .. "/Configs/" .. Splitted[2] .. ".lua") then
                local Table = loadfile("Photon/" .. game.PlaceId .. "/Configs/" .. Splitted[2] .. ".lua")()
                Photon = Table
        ...
        elseif Splitted[1] == Photon.ChatCommands.SaveConfig then
            if not isfolder("Photon") then
                makefolder("Photon")
            end
            if not isfolder("Photon/" .. game.PlaceId) then
                makefolder("Photon/" .. game.PlaceId)
            end
            if not isfolder("Photon/" .. game.PlaceId .. "/Configs") then
                makefolder("Photon/" .. game.PlaceId .. "/Configs")
            end
            if not isfile("Photon/" .. game.PlaceId .. "/Configs/" .. Splitted[2] .. ".lua") then
                writefile("Photon/" .. game.PlaceId .. "/Configs/" .. Splitted[2] .. ".lua", Script.Functions.GetTable(Photon))
        ...
        elseif Splitted[1] == Photon.ChatCommands.Silent_Prediction then
            Photon.Silent.Prediction = Splitted[2]
        elseif Splitted[1] == Photon.ChatCommands.Silent_Fov_Size then
            SilentFovRadius.Value = Splitted[2]
        elseif Splitted[1] == Photon.ChatCommands.Silent_Fov_Show then
            if Splitted[2] == ("true") then
                Photon.Silent.Fov.Visible = true
            else
                Photon.Silent.Fov.Visible = false
            end
        elseif Splitted[1] == Photon.ChatCommands.Silent_Enabled then
            if Splitted[2] == ("true") then
                Photon.Silent.Enabled = true
            else
                Photon.Silent.Enabled = false 
            end
        elseif Splitted[1] == Photon.ChatCommands.Silent_HitChance then
            Photon.Silent.HitChance = Splitted[2]
        elseif Splitted[1] == Photon.ChatCommands.Silent_LegitMode then
            if Splitted[2] == ("true") then
                Photon.Silent.LegitMode = true
            else
                Photon.Silent.LegitMode = false
            end
        elseif Splitted[1] == Photon.ChatCommands.Silent_BlatantMode then
            if Splitted[2] == ("true") then
                Photon.Silent.BlatantMode = true
            else
                Photon.Silent.BlatantMode = false
            end
        elseif Splitted[1] == Photon.ChatCommands.AimAssist_Prediction then
            Photon.AimAssist.Prediction = Splitted[2]
        elseif Splitted[1] == Photon.ChatCommands.AimAssist_Fov_Size then
            Photon.AimAssist.Fov.Radius = Splitted[2]
        elseif Splitted[1] == Photon.ChatCommands.AimAssist_Fov_Show then
            if Splitted[2] == ("true") then
                Photon.AimAssist.Fov.Visible = true
            else
                Photon.AimAssist.Fov.Visible = false
            end
        elseif Splitted[1] == Photon.ChatCommands.AimAssist_Enabled then
            if Splitted[2] == ("true") then
                Photon.AimAssist.Enabled = true
            else
                Photon.AimAssist.Enabled = false
            end
        elseif Splitted[1] == Photon.ChatCommands.AimAssist_SmoothX then
            Photon.AimAssist.Smoothness_X = Splitted[2]
            Script.SavedValue.AimAssistSmoothX = Photon.AimAssist.Smoothness_X
        elseif Splitted[1] == Photon.ChatCommands.AimAssist_SmoothY then
            Photon.AimAssist.Smoothness_Y = Splitted[2]
            Script.SavedValue.AimAssistSmoothY = Photon.AimAssist.Smoothness_Y
        elseif Splitted[1] == Photon.ChatCommands.AimAssist_Shake then
            Photon.AimAssist.Shake.Shake_X = Splitted[2]
            Photon.AimAssist.Shake.Shake_Y = Splitted[2]
            Photon.AimAssist.Shake.Shake_Z = Splitted[2]
            Script.SavedValue.ShakeX = Photon.AimAssist.Shake.Shake_X
            Script.SavedValue.ShakeY = Photon.AimAssist.Shake.Shake_Y
            Script.SavedValue.ShakeZ = Photon.AimAssist.Shake.Shake_Z
        end
...
Uis.InputBegan:connect(function(input, Gp)
    if not Gp then
        ...
        if input.KeyCode == Enum.KeyCode[string.upper(Photon.InventorySorter.KeyBind)] and Photon.InventorySorter.Enabled and Script.Functions.Alive(Client) then
            local GunOrder = Photon.InventorySorter.Slots
            ...
            if TimeSinceLastKeybind >= 5 then
                ...
                if Photon.InventorySorter.UseFood then
        ...
        if Photon.FakeSpike.Enabled and input.KeyCode == Enum.KeyCode[string.upper(Photon.FakeSpike.KeyBind)] then
            if Photon.FakeSpike.ToggleMode then
                FakeSpike = not FakeSpike
                if FakeSpike == true then
                    settings().Network.IncomingReplicationLag = (Photon.FakeSpike.Power * 0.001)
                    if Photon.Options.NotificationMode.Enabled and Photon.Options.NotificationMode.FakeSpike then
                ...
                task.wait(Photon.FakeSpike.Delay)
                settings().Network.IncomingReplicationLag = 0
                if Photon.Options.NotificationMode.Enabled and Photon.Options.NotificationMode.FakeSpike then
                    Script.Functions.CreateNotification("FakeSpike: " .. tostring(FakeSpike), Color3.fromRGB(206, 67, 67))
                end
        ...
        if Photon.F9Cleaner.Enabled and input.KeyCode == Enum.KeyCode[string.upper(Photon.F9Cleaner.KeyBind)] then
            Script.Functions.ClearConsole()
        end

        if input.KeyCode == Enum.KeyCode[string.upper(Photon.AimAssist.FrameSkip.KeyBind)] and Script.Functions.Alive(AimTarget) and Photon.AimAssist.FrameSkip.Enabled then
            FrameSkip = true
        end

        if input.KeyCode == Enum.KeyCode[string.upper(Photon.Silent.ForceKeyBind)] and Photon.Silent.ForceLock_AimAssistTarget == false and Photon.Silent.ForceLock then
            if ForceLock == nil then
                ForceLock = Script.Functions.GetClosestPlayer(true)
        		if Photon.Options.NotificationMode.Enabled and Photon.Options.NotificationMode.Misc and ForceLock ~= nil then
        		    Script.Functions.CreateNotification("Locked: " .. tostring(ForceLock), Color3.fromRGB(206, 67, 67))
        		end
            else
                if ForceLock ~= nil then
                    ForceLock = nil
            		if Photon.Options.NotificationMode.Enabled and Photon.Options.NotificationMode.Misc then
        ...
        if input.KeyCode == Enum.KeyCode.I and Photon.Macro.Speed_MacroAbuse and Script.Functions.Alive(Client) then
        ...
        if input.KeyCode == Enum.KeyCode.O and Photon.Macro.Speed_MacroAbuse and Script.Functions.Alive(Client) then
            if Client.Character:FindFirstChild("GunScript", true) ~= nil or Client.Character:FindFirstChild("FlameThrowerScript", true) ~= nil or Client.Character:FindFirstChild("RPGScript", true) ~= nil then
                local Controller = require(Client:WaitForChild("PlayerScripts"):WaitForChild("PlayerModule")):GetCameras().activeCameraController
                Controller:SetCameraToSubjectDistance(Controller.currentSubjectDistance + 5)
            end
        ...
        if input.KeyCode == Enum.KeyCode[string.upper(Photon.AimAssist.KeyBind)] and Photon.AimAssist.Enabled then
            if AimTarget == nil then
                CurrentVelocity2 = Vector3.zero
                AimTarget = Script.Functions.GetClosestPlayer2()
                if Photon.Options.NotificationMode.Enabled and Photon.Options.NotificationMode.AimAssist and AimTarget ~= nil then
                ...
                if AimTarget ~= nil then
                    AimTarget = nil
                    CurrentVelocity2 = Vector3.zero
                    if Photon.Options.NotificationMode.Enabled and Photon.Options.NotificationMode.AimAssist then
                        Script.Functions.CreateNotification("Unlocked", Color3.fromRGB(206, 67, 67))
                    end
                end
            end
        ...
        if Photon.Silent.TriggerBot and Photon.Silent.TriggerBot_HotKey == false and input.UserInputType == Enum.UserInputType[Photon.Silent.TriggerBotMouseKey] then
            TriggerBot = not TriggerBot
        end

        if Photon.Silent.TriggerBot and Photon.Silent.TriggerBot_HotKey and input.KeyCode == Enum.KeyCode[Photon.Silent.TriggerBotKey] then
            TriggerBot = not TriggerBot
        end

        if input.KeyCode == Enum.KeyCode[string.upper(Photon.Silent.KeyBind)] and Photon.Silent.UseSilentKeyBind then
            Photon.Silent.Enabled = not Photon.Silent.Enabled
            if Photon.Options.NotificationMode.Enabled and Photon.Options.NotificationMode.Silent then
                Script.Functions.CreateNotification("Silent Aim: " .. tostring(Photon.Silent.Enabled), Color3.fromRGB(206, 67, 67))
            end
            CurrentVelocity = Vector3.zero
        end

        if input.KeyCode == Enum.KeyCode[string.upper(Photon.Macro.Lay_KeyBind)] and Photon.Macro.Lay_Emote and game.PlaceId ~= 9825515356 then
            local Args = {
                [1] = "AnimationPack",
                [2] = "Lay"
            }
        ...
        if input.KeyCode == Enum.KeyCode[string.upper(Photon.Macro.Greet_Keybind)] and Photon.Macro.Greet_Emote and game.PlaceId ~= 9825515356 then
            local Args = {
                [1] = "AnimationPack",
                [2] = "Greet"
            }
        ...
        if input.KeyCode == Enum.KeyCode[string.upper(Photon.Esp.EspKey)] and Photon.Esp.UseEspKeyBind then
    		Photon.Esp.Enabled = not Photon.Esp.Enabled
    		if Photon.Options.NotificationMode.Enabled and Photon.Options.NotificationMode.Esp then
    		    Script.Functions.CreateNotification("Esp: " .. tostring(Photon.Esp.Enabled), Color3.fromRGB(206, 67, 67))
    		end
        end

        if input.KeyCode == Enum.KeyCode[string.upper(Photon.Desync.DesyncKey)] and Photon.Desync.Enabled and Photon.Desync.UseDesyncKey then
    		Desync = not Desync
    		if Photon.Options.NotificationMode.Enabled and Photon.Options.NotificationMode.Esp then
    		    Script.Functions.CreateNotification("Desync: " .. tostring(Desync), Color3.fromRGB(206, 67, 67))
    		end
            if Photon.Desync.Method == ("Slow_Data") then
                wait()
                setfflag("S2PhysicsSenderRate", 15)
            end
        end

        if input.KeyCode == Enum.KeyCode[string.upper(Photon.Macro.Noclip_KeyBind)] and Photon.Macro.Noclip_Macro then
        ...
        if input.KeyCode == Enum.KeyCode[string.upper(Photon.Macro.Speed_KeyBind)] and Photon.Macro.Speed_Enabled then
            Macro = not Macro
            repeat RS.Heartbeat:Wait()
            if Photon.Macro.Speed_Method == ("FirstPerson") then
                local Controller = require(Client:WaitForChild("PlayerScripts"):WaitForChild("PlayerModule")):GetCameras().activeCameraController
                Controller:SetCameraToSubjectDistance(Controller.currentSubjectDistance - 1)
                for i = 1, math.ceil(Photon.Macro.Speed_Delay) do
                    RS.Heartbeat:Wait()
                end
                Controller:SetCameraToSubjectDistance(Controller.currentSubjectDistance + 1)
            elseif Photon.Macro.Speed_Method == ("Shift") then
                keypress(0xA0)
                for i = 1, math.ceil(Photon.Macro.Speed_Delay) do
                    RS.Heartbeat:Wait()
                end
                ...
                keyrelease(0xA0)
            elseif Photon.Macro.Speed_Method == ("ThirdPerson") then
                if Photon.Macro.Speed_ThirdPersonV2 then
                    local Controller = require(Client:WaitForChild("PlayerScripts"):WaitForChild("PlayerModule")):GetCameras().activeCameraController
        ...
        if input.KeyCode == Enum.KeyCode[string.upper(Photon.PanicMode.KeyBind)] and Photon.PanicMode.Enabled then
            PanicMode = not PanicMode
            if PanicMode then
                if Photon.Options.NotificationMode.Enabled and Photon.Options.NotificationMode.PanicMode then
                    Script.Functions.CreateNotification("PanicMode: " .. tostring(PanicMode), Color3.fromRGB(206, 67, 67))
                end
                if Photon.Options.NotificationMode == true then
                    Script.PanicModeSaves.CurrentNotificationState = Photon.Options.NotificationMode
                    Photon.Options.NotificationMode = not Photon.Options.NotificationMode
                end
                if Photon.Silent.Enabled == true then
                    Script.PanicModeSaves.CurrentSilentState = Photon.Silent.Enabled
                    Photon.Silent.Enabled = not Photon.Silent.Enabled
                end
                if Photon.AimAssist.Enabled == true then
                    Script.PanicModeSaves.CurrentAimAssistState = Photon.AimAssist.Enabled
                    Photon.AimAssist.Enabled = not Photon.AimAssist.Enabled
                end
                if Photon.AimAssist.Fov.Visible == true then
                    Script.PanicModeSaves.CurrentAimAssistFov = Photon.AimAssist.Fov.Visible
                    Photon.AimAssist.Fov.Visible = not Photon.AimAssist.Fov.Visible
                end
                if Photon.Silent.Fov.Visible == true then
                    Script.PanicModeSaves.CurrentSilentFov = Photon.Silent.Fov.Visible
                    Photon.Silent.Fov.Visible = not Photon.Silent.Fov.Visible
                end
            else
                ...
                if Script.PanicModeSaves.CurrentSilentState then
                    Photon.Silent.Enabled = Script.PanicModeSaves.CurrentSilentState
                end
                if Script.PanicModeSaves.CurrentAimAssistState then
                    Photon.AimAssist.Enabled = Script.PanicModeSaves.CurrentAimAssistState
                end
                if Script.PanicModeSaves.CurrentAimAssistFov then
                    Photon.AimAssist.Fov.Visible = Script.PanicModeSaves.CurrentAimAssistFov
                end
                if Script.PanicModeSaves.CurrentSilentFov then
                    Photon.Silent.Fov.Visible = Script.PanicModeSaves.CurrentSilentFov
                end
            end
        end

        if input.KeyCode == Enum.KeyCode[string.upper(Photon.Macro.Rotation_KeyBind)] and Photon.Macro.RotationMode then
            if Photon.AimAssist.Enabled then
                Photon.AimAssist.Enabled = false
            end
            for i = 1, math.floor(Photon.Macro.Degrees / Photon.Macro.RotationSpeed) do
                Camera.CoordinateFrame = Camera.CoordinateFrame * CFrame.Angles(0, math.rad(Photon.Macro.RotationSpeed), 0)
                RS.Heartbeat:Wait()
            end
            if Photon.AimAssist.Enabled then
                Photon.AimAssist.Enabled = true
...
Uis.InputEnded:connect(function(input, Gp)
    if not Gp then
        if Photon.Silent.TriggerBot and TriggerBot and Photon.Silent.TriggerBot_HotKey == false and Photon.Silent.TriggerBot_HoldMode and input.UserInputType == Enum.UserInputType[Photon.Silent.TriggerBotMouseKey] then
            TriggerBot = false
        end

        if Photon.Silent.TriggerBot and TriggerBot and Photon.Silent.TriggerBot_HotKey and Photon.Silent.TriggerBot_HoldMode and input.KeyCode == Enum.KeyCode[string.upper(Photon.Silent.TriggerBotKey)] then
            TriggerBot = false
        end

        if input.KeyCode == Enum.KeyCode[string.upper(Photon.Esp.EspKey)] and Photon.Esp.UseEspKeyBind and Photon.Esp.HoldMode and Photon.Esp.Enabled then
    		Photon.Esp.Enabled = false
    		if Photon.Options.NotificationMode.Enabled and Photon.Options.NotificationMode.Esp then
    		    Script.Functions.CreateNotification("Esp: " .. tostring(Photon.Esp.Enabled), Color3.fromRGB(206, 67, 67))
    		end
        end

        if input.KeyCode == Enum.KeyCode[string.upper(Photon.AimAssist.KeyBind)] and Photon.AimAssist.Enabled and Photon.AimAssist.HoldMode then
    		AimTarget = nil
    		CurrentVelocity2 = Vector3.zero
        end

        if input.KeyCode == Enum.KeyCode[string.upper(Photon.Desync.DesyncKey)] and Photon.Desync.HoldMode and Photon.Desync.UseDesyncKey and Photon.Desync.Enabled then
    		Desync = false
    		if Photon.Options.NotificationMode.Enabled and Photon.Options.NotificationMode.Desync then
    		    Script.Functions.CreateNotification("Desync: " .. tostring(Desync), Color3.fromRGB(206, 67, 67))
    		end
        end

        if input.KeyCode == Enum.KeyCode[string.upper(Photon.Macro.Speed_KeyBind)] and Photon.Macro.Speed_Enabled and Photon.Macro.Speed_HoldMode and Macro then
            Macro = false
        end

        if input.KeyCode == Enum.KeyCode[string.upper(Photon.Macro.Noclip_KeyBind)] and Photon.Macro.Noclip_Macro and Photon.Macro.Noclip_HoldMode and NoclipMacro then
            NoclipMacro = false
        end
    end
end)
...
Script.Functions.ToolActivated = function()
    if Photon.Silent.Enabled and Photon.Silent.AntiAimViewer and Script.Functions.Alive(SilentTarget) and Script.Functions.SilentCheck(SilentTarget) then
        local TargetCF = nil
        local TargetFalling = false
        ...
        if TargetCF and CurrentVelocity then
            if Photon.Silent.PredictMovement then 
                if Photon.Silent.BlatantMode then
                    ...
                    if Enabled and Photon.UniversalCheck.WallCheck_V2 and not Script.Functions.WallCheck(SilentTarget.Character.HumanoidRootPart.Position + (SilentVel * Photon.Silent.Prediction), SilentTarget.Character) then
                        return
                    end
                    if Enabled then
                        if TargetFalling then
                            TargetCF = TargetCF + (Vector3.new(SilentVel.X, (SilentVel.Y * Photon.Silent.AntiGroundValue), SilentVel.Z) * Photon.Silent.Prediction)
                        else
                            TargetCF = TargetCF + (SilentVel * Photon.Silent.Prediction)
                        end
                    else
                        if TargetFalling then
                            TargetCF = TargetCF + (Vector3.new(CurrentVelocity.X, (CurrentVelocity.Y * Photon.Silent.AntiGroundValue), CurrentVelocity.Z) * Photon.Silent.Prediction)
                        else
                            TargetCF = TargetCF + (CurrentVelocity * Photon.Silent.Prediction)
                        end
                    end
                else
                    if TargetFalling then
        ...
        if TargetCF then
            ...
            if Photon.Silent.Custom_AntiAimViewerPoint.Enabled == false then
                ...
                if type(Photon.Silent.Custom_AntiAimViewerPoint.RemoteEvent) == ("function") then
                    local Args, MainEvent = Photon.Silent.Custom_AntiAimViewerPoint.RemoteEvent(TargetCF)
...
coroutine.resume(coroutine.create(function()
    while Photon.MemorySpoofer.Enabled and RS.Heartbeat:Wait() do
        Script.Functions.ChangeText()
    end
end))
    
coroutine.resume(coroutine.create(function()
    while Photon.MemorySpoofer.Enabled do
        task.wait(Photon.MemorySpoofer.Delay)
        Text_1 = tostring(math.random(Photon.MemorySpoofer.Lowest, Photon.MemorySpoofer.Maximum))
        Text_2 = tostring("." .. math.random(10, 99) .. " MB")
        Text_3 = tostring(math.random(Photon.MemorySpoofer.Lowest, Photon.MemorySpoofer.Maximum))
...
coroutine.resume(coroutine.create(function()
    while Photon.MemorySpoofer.Enabled do
        task.wait(5)
        Text_5 = tostring(math.random(Photon.MemorySpoofer.Lowest, Photon.MemorySpoofer.Maximum))
...
coroutine.resume(coroutine.create(function()
    while true do
        local Succes, Error = pcall(function()
            ...
            if Photon.Options.AntiError then 
            ...
            if Photon.Options.AutoGetUp and Script.Functions.Alive(Client) and Client.Character.Humanoid:GetState() == Enum.HumanoidStateType.FallingDown then
                Client.Character.Humanoid:ChangeState("GettingUp")
            end
            Script.Functions.MouseChanger()
            if TriggerBot and Script.Functions.Alive(SilentTarget) and SilentTarget.Character:FindFirstChild(Photon.Silent.Part) then
                local Magnitude = Script.Functions.GetMagnitudeFromMouse(SilentTarget.Character.HumanoidRootPart)

                if (Photon.Silent.ForceLock == false and Script.Drawing.SilentCircle.Radius + 4 > Magnitude) or Photon.Silent.ForceLock == true then
                    if Photon.Silent.TriggerBot_Delay == 0 then
                        mouse1click()
                    else
                        task.spawn(function()
                            task.wait(Photon.Silent.TriggerBot_Delay / 1000)
                            mouse1click()
                        end)
                    end
                end
...
coroutine.resume(coroutine.create(function()
    while true do
        local Succes, Error = pcall(function()
            if Photon.UniversalCheck.Advanced.Target_Bots and game:GetService("Workspace"):FindFirstChild(Photon.UniversalCheck.Advanced.Bot_Path) then
                for _, v in pairs(game:GetService("Workspace")[Photon.UniversalCheck.Advanced.Bot_Path]:GetChildren()) do
        	...
        	if Photon.Silent.GunSettings.Enabled then
            	local CurrentGun = Script.Functions.GetCurrentWeaponName()
                local WeaponSettings = Photon.Silent.GunSettings[CurrentGun]
                if WeaponSettings ~= nil then
                    if Photon.Silent.GunSettings.Methods.Range == false then
                        if Photon.Silent.GunSettings.Methods.Smoothness and Script.SavedValue.AimAssistSmoothX ~=  WeaponSettings.Smoothness then
                            Photon.AimAssist.Smoothness_X = WeaponSettings.Smoothness
                            Photon.AimAssist.Smoothness_Y = WeaponSettings.Smoothness
                            Script.SavedValue.AimAssistSmoothX = WeaponSettings.Smoothness
                            Script.SavedValue.AimAssistSmoothY = WeaponSettings.Smoothness
                        end
                        if Photon.Silent.GunSettings.Methods.AirSmoothness and Photon.AimAssist.AirSmoothness_X ~= WeaponSettings.AirSmoothness then
                            Photon.AimAssist.AirSmoothness_X = WeaponSettings.AirSmoothness
                            Photon.AimAssist.AirSmoothness_Y = WeaponSettings.AirSmoothness
                        end
                        if Photon.Silent.GunSettings.Methods.HitChance and Photon.Silent.Prediction ~= WeaponSettings.HitChance then
                            Photon.Silent.HitChance = WeaponSettings.HitChance
                        end
                        if Photon.Silent.GunSettings.Methods.AirHitChance and Photon.Silent.Prediction ~= WeaponSettings.AirHitChance then
                            Photon.Silent.AirHitChance = WeaponSettings.AirHitChance
                        end
                        if Photon.Silent.GunSettings.Methods.Prediction and Photon.Silent.Prediction ~= WeaponSettings.Prediction then
                            Photon.Silent.Prediction = WeaponSettings.Prediction
                        end
                        if Photon.Silent.GunSettings.Methods.Fov and SilentFovRadius.Value ~= WeaponSettings.Fov then
                            if Photon.Silent.GunSettings.Dynamic.Enabled then
                                local Create = Tween:Create(SilentFovRadius, TweenInfo.new(Photon.Silent.GunSettings.Dynamic.Time, Enum.EasingStyle[Photon.Silent.GunSettings.Dynamic.EasingStyle], Enum.EasingDirection[Photon.Silent.GunSettings.Dynamic.EasingDirection]), {Value = WeaponSettings.Fov})
                                Create:Play()
                                Create.Completed:Wait()
                            else
                    ...
                    if Photon.Silent.Part == nil then return end
                    if Photon.Silent.GunSettings.Methods.Range and Script.Functions.Alive(SilentTarget) and Script.Functions.Alive(Client) and SilentTarget.Character:FindFirstChild(Photon.Silent.Part) then
                        local Magnitude = Script.Functions.GetMagnitudeFromMouse(SilentTarget.Character.HumanoidRootPart)

                        if (Photon.Silent.ForceLock == false and Script.Drawing.SilentCircle.Radius + 4 > Magnitude) or Photon.Silent.ForceLock == true then
                            local Magnitude = (SilentTarget.Character.HumanoidRootPart.Position - Client.Character.HumanoidRootPart.Position).Magnitude
                            if Magnitude < Photon.Silent.GunSettings.Close_Activation then
                                if Photon.Silent.GunSettings.Methods.Smoothness and Script.SavedValue.AimAssistSmoothX ~=  WeaponSettings.CloseSmoothness then
                                    Photon.AimAssist.Smoothness_X = WeaponSettings.CloseSmoothness
                                    Photon.AimAssist.Smoothness_Y = WeaponSettings.CloseSmoothness
                                ...
                                if Photon.Silent.GunSettings.Methods.AirSmoothness and Photon.AimAssist.AirSmoothness_X ~= WeaponSettings.CloseAirSmoothness then
                                    Photon.AimAssist.AirSmoothness_X = WeaponSettings.CloseAirSmoothness
                                    Photon.AimAssist.AirSmoothness_Y = WeaponSettings.CloseAirSmoothness
                                end
                                if Photon.Silent.GunSettings.Methods.HitChance and Photon.Silent.HitChance ~= WeaponSettings.CloseHitChance then
                                    Photon.Silent.HitChance = WeaponSettings.CloseHitChance
                                end
                                if Photon.Silent.GunSettings.Methods.AirHitChance and Photon.Silent.AirHitChance ~= WeaponSettings.CloseAirHitChance then
                                    Photon.Silent.AirHitChance = WeaponSettings.CloseAirHitChance
                                end
                                if Photon.Silent.GunSettings.Methods.Prediction and Photon.Silent.Prediction ~= WeaponSettings.ClosePrediction then
                                    Photon.Silent.Prediction = WeaponSettings.ClosePrediction
                                end
                                if Photon.Silent.GunSettings.Methods.Fov and SilentFovRadius.Value ~= WeaponSettings.CloseFov then
                                    if Photon.Silent.GunSettings.Dynamic.Enabled then
                                        local Create = Tween:Create(SilentFovRadius, TweenInfo.new(Photon.Silent.GunSettings.Dynamic.Time, Enum.EasingStyle[Photon.Silent.GunSettings.Dynamic.EasingStyle], Enum.EasingDirection[Photon.Silent.GunSettings.Dynamic.EasingDirection]), {Value = WeaponSettings.CloseFov})
                                        Create:Play()
                                        Create.Completed:Wait()
                                    else
                            ...
                            elseif Magnitude < Photon.Silent.GunSettings.Medium_Activation then
                                if Photon.Silent.GunSettings.Methods.Smoothness and Script.SavedValue.AimAssistSmoothX ~= WeaponSettings.MedSmoothness then
                                    Photon.AimAssist.Smoothness_X = WeaponSettings.MedSmoothness
                                    Photon.AimAssist.Smoothness_Y = WeaponSettings.MedSmoothness
                                ...
                                if Photon.Silent.GunSettings.Methods.AirSmoothness and Photon.AimAssist.AirSmoothness_X ~= WeaponSettings.MedAirSmoothness then
                                    Photon.AimAssist.AirSmoothness_X = WeaponSettings.MedAirSmoothness
                                    Photon.AimAssist.AirSmoothness_Y = WeaponSettings.MedAirSmoothness
                                end
                                if Photon.Silent.GunSettings.Methods.HitChance and Photon.Silent.HitChance ~= WeaponSettings.MedHitChance then
                                    Photon.Silent.HitChance = WeaponSettings.MedHitChance
                                end
                                if Photon.Silent.GunSettings.Methods.AirHitChance and Photon.Silent.AirHitChance ~= WeaponSettings.MedAirHitChance then
                                    Photon.Silent.AirHitChance = WeaponSettings.MedAirHitChance
                                end
                                if Photon.Silent.GunSettings.Methods.Prediction and Photon.Silent.Prediction ~= WeaponSettings.MedPrediction then
                                    Photon.Silent.Prediction = WeaponSettings.ClosePrediction
                                end
                                if Photon.Silent.GunSettings.Methods.Fov and SilentFovRadius.Value ~= WeaponSettings.MedFov then
                                    if Photon.Silent.GunSettings.Dynamic.Enabled then
                                        local Create = Tween:Create(SilentFovRadius, TweenInfo.new(Photon.Silent.GunSettings.Dynamic.Time, Enum.EasingStyle[Photon.Silent.GunSettings.Dynamic.EasingStyle], Enum.EasingDirection[Photon.Silent.GunSettings.Dynamic.EasingDirection]), {Value = WeaponSettings.MedFov})
                                        Create:Play()
                                        Create.Completed:Wait()
                                    else
                            ...
                            elseif Magnitude < Photon.Silent.GunSettings.Far_Activation then
                                if Photon.Silent.GunSettings.Methods.Smoothness and Script.SavedValue.AimAssistSmoothX ~= WeaponSettings.FarSmoothness then
                                    Photon.AimAssist.Smoothness_X = WeaponSettings.FarSmoothness
                                    Photon.AimAssist.Smoothness_Y = WeaponSettings.FarSmoothness
                                ...
                                if Photon.Silent.GunSettings.Methods.AirSmoothness and Photon.AimAssist.AirSmoothness_X ~= WeaponSettings.FarAirSmoothness then
                                    Photon.AimAssist.AirSmoothness_X = WeaponSettings.FarAirSmoothness
                                    Photon.AimAssist.AirSmoothness_Y = WeaponSettings.FarAirSmoothness
                                end
                                if Photon.Silent.GunSettings.Methods.HitChance and Photon.Silent.HitChance ~= WeaponSettings.FarHitChance then
                                    Photon.Silent.HitChance = WeaponSettings.FarHitChance
                                end
                                if Photon.Silent.GunSettings.Methods.AirHitChance and Photon.Silent.AirHitChance ~= WeaponSettings.FarAirHitChance then
                                    Photon.Silent.AirHitChance = WeaponSettings.FarAirHitChance
                                end
                                if Photon.Silent.GunSettings.Methods.Prediction and Photon.Silent.Prediction ~= WeaponSettings.FarPrediction then
                                    Photon.Silent.Prediction = WeaponSettings.FarPrediction
                                end
                                if Photon.Silent.GunSettings.Methods.Fov and SilentFovRadius.Value ~= WeaponSettings.FarFov then
                                    if Photon.Silent.GunSettings.Dynamic.Enabled then
                                        local Create = Tween:Create(SilentFovRadius, TweenInfo.new(Photon.Silent.GunSettings.Dynamic.Time, Enum.EasingStyle[Photon.Silent.GunSettings.Dynamic.EasingStyle], Enum.EasingDirection[Photon.Silent.GunSettings.Dynamic.EasingDirection]), {Value = WeaponSettings.FarFov})
                                        Create:Play()
                                        Create.Completed:Wait()
                                    else
...
if Photon.Options.Internal and rconsoleprint and rconsolename then
    local InternalChatCommands = {
        ...
        discord = function(Plr)
            if setclipboard then
                setclipboard("discord.gg/photon")
            end
...
if Photon.Options.GetInformation then
    if GetTime then
